...
export default function GivingPlanner({ onClose, onCreate }: { onClose: () => void; onCreate: (amount: string, label: string) => void }) {
  ...
  return <div className="overlay planner-overlay" role="dialog" aria-modal="true" aria-label="Giving planner">
    <div className="planner-panel">
      ...
      <div className="planner-intro"><span className="planner-orbit"><i>☾</i></span><p className="eyebrow light">Your giving rhythm</p><h2>Turn good intentions into a gentle habit.</h2><p>Choose an occasion, amount and cause. You stay in control and can pause any plan at any time.</p></div>
      <div className="planner-form">
        ...
        <div className="occasion-grid">{occasions.map(item => <button key={item.id} className={occasion === item.id ? 'selected' : ''} onClick={() => { setOccasion(item.id); setAmount(String(item.defaultAmount)); }}><i>{item.icon}</i><span><b>{item.title}</b><small>{item.note}</small></span></button>)}</div>
        ...
        <button className="planner-toggle" onClick={() => setReminders(!reminders)}><span><b>Gentle reminders</b><small>Receive a notice before each scheduled gift.</small></span><i className={reminders ? 'on' : ''}><b/></i></button>
        ...
        <button className="button full planner-create" disabled={!Number(amount)} onClick={() => onCreate(amount, selected.title)}>Continue securely <span>↗</span></button>
