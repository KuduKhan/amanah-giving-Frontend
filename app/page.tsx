...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') { setSearchOpen(false); setSelected(null); setDashboardOpen(false); setPlatformOpen(false); setPlannerOpen(false); setNotificationsOpen(false); }
    };
  ...
  return (
    <main>
      ...
      {notificationsOpen && <div className="overlay notification-overlay" role="dialog" aria-modal="true" aria-label="Notifications"><div className="notification-panel"><button className="close" onClick={() => setNotificationsOpen(false)}>×</button><div className="notification-head"><div><p className="eyebrow">Notification center</p><h2>Peace of mind, delivered.</h2></div><button onClick={() => setToast('All notifications marked as read')}>Mark all read</button></div><div className="notification-tabs"><button className="active">All <span>3</span></button><button>Impact</button><button>Payments</button><button>Plans</button></div><div className="notification-list">{[
