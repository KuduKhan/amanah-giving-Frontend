...
function DonorPortal({ onGive, act, notifications, setNotifications, privateProfile, setPrivateProfile }: DonorProps) {
  return <>
    ...
    <div className="portal-grid donor-grid">
      <section className="portal-card trace-card"><CardHead eyebrow="GIFT JOURNEY" title="Trace gift AMN-948231" action="Full evidence →"/><div className="trace-route">{[['✓','Payment','Confirmed'],['✓','Fund','Restricted'],['✓','Approval','3 of 3'],['✓','Delivery','Completed'],['✓','Impact','340 people']].map((x,i) => <div key={x[1]} className="trace-node"><i><Icon name={x[0]}/></i><b>{x[1]}</b><small>{x[2]}</small>{i < 4 && <span/>}</div>)}</div><div className="trace-detail"><img src="/water-community.jpg" alt="Completed water project"/><div><span>WTR-284 · GARISSA</span><b>Clean water is flowing</b><p>Water test, contractor invoices, completion photos and beneficiary count verified.</p><button onClick={() => act('Evidence bundle opened')}>View 7 evidence files <Icon name="↗"/></button></div></div></section>
