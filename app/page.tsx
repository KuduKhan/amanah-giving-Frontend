...
const campaigns: Campaign[] = [
  ...
  { id: 'HLT-728', title: 'Emergency care for vulnerable families', category: 'Health', meta: 'Kilifi, Kenya', raised: 1480000, goal: 2200000, donors: 531, days: 9, image: '/healthcare-community.jpg', badge: 'Urgent care', zakat: true, story: 'Help verified households access urgent consultations, essential medicine and safe transport to partner health facilities.' },
  { id: 'EDU-390', title: 'Equip a community Qur’an learning centre', category: 'Education', meta: 'Nakuru, Kenya', raised: 925000, goal: 1600000, donors: 318, days: 19, image: '/quran-learning.jpg', badge: 'Sadaqah Jariyah', zakat: false, story: 'Provide Qur’ans, desks, lighting and accessible learning materials for children and adult students in the local community.' },
];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!stored) return;
    const frame = requestAnimationFrame(() => {
      try { setSaved(JSON.parse(stored)); } catch { localStorage.removeItem('amanah-saved'); }
    });
  ...
  function startDonation(campaign?: Campaign | null, preset?: string) {
    ...
    <main id="main-content">
      ...
      <section className="hero" id="top">
        ...
        <div className="hero-visual">
          <Image src="/water-community.jpg" alt="Children gathering at a rural community water well" width={1400} height={1200} priority sizes="(max-width: 900px) 100vw, 50vw" />
      ...
      <section className="section" id="causes">
        ...
        <div className="filter-row" aria-label="Filter verified causes">{['All','Zakat','Food','Orphans','Water','Mosque'].map(item => <button className={filter === item ? 'active' : ''} aria-pressed={filter === item} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div>
        <div className="campaign-grid">
          {filteredCampaigns.map(campaign => {
            const progress = Math.round(campaign.raised / campaign.goal * 100);
            return <article className="campaign-card" key={campaign.id}>
              <div className="campaign-image"><Image src={campaign.image} alt="" width={900} height={600} sizes="(max-width: 700px) 100vw, (max-width: 1100px) 50vw, 33vw" style={{ objectPosition: campaign.id === 'EDU-390' ? '50% 34%' : 'center' }} /><span className="badge">{campaign.badge}</span><button className={`save ${saved.includes(campaign.id) ? 'saved' : ''}`} aria-pressed={saved.includes(campaign.id)} onClick={() => toggleSaved(campaign.id)} aria-label={`${saved.includes(campaign.id) ? 'Remove' : 'Save'} ${campaign.title}`}>{saved.includes(campaign.id) ? <Icon name="♥"/> : <Icon name="♡"/>}</button></div>
        ...
        {!filteredCampaigns.length && <div className="empty-state" role="status">No verified campaigns match this search yet.</div>}
      </section>
      ...
      <section className="updates-section">
        ...
        <div className="updates-grid">
          {[
            ...
            <button className="update-image" onClick={() => setSelected(update.campaign)} aria-label={`View ${update.title}`}><Image src={update.campaign.image} alt="" width={760} height={500} sizes="(max-width: 780px) 42vw, 33vw"/><span>{update.label}</span></button>
      ...
      <footer><div className="footer-brand"><a className="brand inverse" href="#top" aria-label="Amanah Giving home"><AmanahLogo variant="light" size="md"/></a><p>Transparent, secure and Shariah-aware digital giving.</p></div><div><b>Give</b><a href="#causes">Urgent appeals</a><a href="#zakat">Zakat Center</a><a href="#impact">Impact Center</a><a href="/sadaqah">Sadaqah giving</a></div><div><b>Platform</b><a href="#how">How it works</a><a href="/about">About Amanah</a><a href="/verified-charities">Verified charities</a><button onClick={() => setPlatformOpen(true)}>Request assistance</button><button onClick={() => setPlatformOpen(true)}>For organizations</button></div><div><b>Trust</b><a href="/faq">FAQ</a><a href="/contact">Contact</a><a href="/policies">Privacy and terms</a><button onClick={() => setPlatformOpen(true)}>Shariah governance</button></div><div className="footer-cta"><p>Ready to make an impact?</p><button className="button sand" onClick={() => startDonation()}>Give now <Icon name="↗"/></button></div></footer>
      ...
      {searchOpen && <div className="overlay" role="dialog" aria-modal="true" aria-label="Search verified campaigns"><div className="search-panel"><button className="close" aria-label="Close dialog" onClick={() => setSearchOpen(false)}><Icon name="×"/></button><p className="eyebrow">Search Amanah Giving</p><h2>Find a cause close to your heart.</h2><div className="search-input"><span><Icon name="⌕"/></span><input autoFocus value={query} onChange={event => setQuery(event.target.value)} aria-label="Search campaigns" placeholder="Try ‘orphan Nairobi’ or ‘water Garissa’" /></div><div className="filter-row" aria-label="Filter search results">{['All','Zakat','Food','Orphans','Water','Mosque'].map(item => <button className={filter === item ? 'active' : ''} aria-pressed={filter === item} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div><div className="search-results">{filteredCampaigns.map(campaign => <button key={campaign.id} onClick={() => { setSearchOpen(false); setSelected(campaign); }}><img src={campaign.image} alt="" loading="lazy" decoding="async"/><span><b>{campaign.title}</b><small>{campaign.meta} · {campaign.zakat ? 'Zakat eligible' : campaign.category}</small></span><em>{Math.round(campaign.raised / campaign.goal * 100)}%</em></button>)}</div>{!filteredCampaigns.length && <div className="empty-state" role="status">No verified campaigns match your search.</div>}</div></div>}
      ...
      {toast && <div className="toast" role="status" aria-live="polite"><span><Icon name="✓"/></span>{toast}</div>}
