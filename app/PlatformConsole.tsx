  ...
  return <div className="platform-overlay" role="dialog" aria-modal="true" aria-label="Amanah platform console">
    <div className="platform-shell">
      ...
      <div className="platform-main">
        <header className="platform-topbar"><div><p>{portalTitle.label} portal</p><h2>{portalTitle.role}</h2></div><div><button className="platform-ai" onClick={() => setAssistantOpen(!assistantOpen)}>✦ Giving assistant</button><button className="platform-close" onClick={onClose} aria-label="Close platform">×</button></div></header>
...
function Toggle({ label, checked, setChecked }: any) { return <button className="toggle-row" onClick={() => setChecked(!checked)}><span>{label}</span><i className={checked ? 'on' : ''}><b/></i></button>; }
